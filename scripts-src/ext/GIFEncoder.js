...
GIFEncoder = function()
{
    ...
    /*private*/ var palettizer = exactPalette;
    ...
    var setPalette = exports.setPalette = function( f ){
	palettizer = f;
    };
    ...
    {
	try{
	    indexedPixels = new Uint8Array((image.length / 4) |0);
	}catch(e){
	    indexedPixels = [];
	};
	colorTab = palettizer( image, indexedPixels );
	colorDepth = 8;
	palSize = 7;
	...
	for (var i/*int*/ = 0; i < len;) {
	    ...
	    var dg/*int*/ = g - (colorTab[i+1] & 0xff);
	    var db/*int*/ = b - (colorTab[i+2] & 0xff);
